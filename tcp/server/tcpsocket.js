...
Socket.prototype.send = function(msg) {
  var result = this._socket.write(msg);
};
...
var readBody = function(socket, data, offset) {
  ...
  if(socket.packageOffset === socket.packageSize) {
    ...
    logger.error('close the connection with invalid body message, the remote ip is %s && port is %s && message is %j', socket._socket.remoteAddress, socket._socket.remotePort, data);
    socket.close();
  }
